    ...
    module AnswerMethods
      ...
      def integer_conditions
        ...
        validations.map { |v| v.validation_conditions }.flatten.each do |condition|
          case condition.operator
            when "<=" then
              rules.merge!({ 'rule-max' => condition.integer_value })
            when "<" then
              rules.merge!({ 'rule-max' => (condition.integer_value + 1) })
            when ">" then
              rules.merge!({ 'rule-min' => (condition.integer_value - 1) })
            when ">=" then
              rules.merge!({ 'rule-min' => condition.integer_value })
            when "==" then
              rules.merge!({ 'rule-equalto' => condition.integer_value })
          end
        end
        rules
      end
      ...
      def text_conditions
        ...
        validations.map { |v| v.validation_conditions }.flatten.each do |condition|
          case condition.operator
            when "=~" then
              rules.merge!({ 'rule-pattern' => condition.regexp })
          end
        end
        rules
      end
