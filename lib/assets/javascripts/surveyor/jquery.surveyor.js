...
jQuery(document).ready(function () {
  ...
  $.extend(SurveyorAjax, {
    ...
    prototype: {
      ...
      fieldsetData: function (element) {
        // console.log('in fieldsetData... ');
        const parents = element.parents('fieldset[id^="q_"],tr[id^="q_"]');
        // console.log('   parents: ', parents);
        const found = parents.find("input, select, textarea");
        // console.log('   found: ', found);
        const found_added = found.add($("form#survey_form input[name='authenticity_token']"));
        // console.log('   found_added: ', found_added);
        const serialized = found_added.serialize();
      ...
      sendData: function (element) {
        ...
        element.data('changeTimeOut', setTimeout(function () {
          $this.setOption("beforeSend", function (xhr) {
            $this.beforeSend(xhr, element);
          });
          $this.setOption("data", $this.fieldsetData(element));
          $.ajax($this.options).done(function (data) {
            $this.done(data, element);
          });
      ...
      showOrHideQuestions: function (responseText) {
        ...
        jQuery.each(responseText.hide, function () {
          ...
          if (response_element.length > 0) {
            ...
            // clear answers for checkboxes and radio buttons
            if (element) {
            ...
            // collapse all collapsable things
            ...
            // hide this element (ex: for response_text = "q_5" hide #q_5)
      ...
      hiddenClassName: function (id) {
        return this.isGroup(id) ? "g_hidden" : "q_hidden"
      },
      isGroup: function (id) {
        return id.match('^g_') ? true : false
      },
      show: function (id, className) {
        $("#" + id).removeClass(className);
      },
      hide: function (id, className) {
      ...
      removeUselessResponse: function (id) {
        if (!this.isGroup(id)) {
          $this = this;
          var id = id.match(/\d+/);
          url_without_params = this.options.url.match(/^([^?]*)/);
          url_params = this.options.url.match(/([?]+)([^?]*)$/);
          url_params = url_params ? url_params[2] : '';  // if there is a ? , get the parameters after it

          $.ajax({
            type: "DELETE",
            dataType: 'json',
            url: url_without_params[0] + "/" + id + "?" + url_params,
  ...
  $(".surveyor_language_selection select#locale").change(function () {
    this.form.submit();
  });
